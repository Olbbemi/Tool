...
C_Parser::C_Parser(TCHAR* pa_path) : m_index(1)
{
	wifstream lo_read_stream;

	StringCchCopy(m_log_action, _countof(m_log_action), _TEXT("Parser"));
	StringCchCopy(m_log_server, _countof(m_log_server), _TEXT("Common"));

	lo_read_stream.open(pa_path, ios_base::binary);
...
void C_Parser::M_Find_Scope(const TCHAR* pa_str)
{
	m_KMP_table.resize(_tcslen(pa_str));
	fill(m_KMP_table.begin(), m_KMP_table.end(), 0);
	...
	while (1)
	{
		...
		else
		{
			...
			{
				ST_Log lo_log{ "Can't Find Scope" };
				_LOG(__LINE__, LOG_LEVEL_SYSTEM, m_log_action, m_log_server, lo_log.count, lo_log.log_str);
			}
		}
...
void C_Parser::M_Get_String(const TCHAR* pa_str, char* pa_data, int pa_size)
{
	M_Find_Str(pa_str);
	WideCharToMultiByte(CP_UTF8, 0, M_Get_Data(), pa_size, pa_data, pa_size, NULL, NULL);
}